...
check("Category list renders", doc.querySelectorAll(".category-pill").length === 25); // 24 categories + ALL
check("'All Categories' pill shown with full total", doc.querySelector(".category-pill")?.textContent.includes("296"));
...
check("Results list renders all cards by default", doc.querySelectorAll(".result-card").length === 296);
check("Result count text shows total", doc.getElementById("resultCount")?.textContent.includes("296"));

const search = doc.getElementById("search");
search.value = "espresso";
search.dispatchEvent(new window.Event("input", { bubbles: true }));
await new Promise(r => setTimeout(r, 50));
check("Typing 'espresso' filters results down", doc.querySelectorAll(".result-card").length < 296);
